                ...
                <div class="address-form">
                  ...
                  <div class="module-form-row clear">
                    <div class="form-item-v3 select-item province-wrapper">
                      <select name="province_code" v-model="receive.provinceId" class="province select-province js-form-province js-verify">
                        <option value="0">请选择省份</option>
                        <option v-for="(province,index) in addList" :value="province.area_id" :key="index">{{province.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row clear">
                    <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="receive.cityId">
                        ...
                        <option v-for="(city,index) in cityList" :value="city.area_id" :key="index">{{city.area_name}}</option>
                      </select>
                    </div>
                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="receive.countyId">
                        ...
                        <option v-for="(county,index) in countyList" :value="county.area_id" :key="index">{{county.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row">
...
export default {
  ...
  computed: {
    ...
    cityList () {
      let cityList = []
      let that = this
      that.addList.forEach((province) => {
        if (province.area_id === this.receive.provinceId) {
          that.receive.province = province.area_name
          cityList = province.city_list
          return
        }
      })
      if (that.receive.provinceId === '0') {
        that.receive.cityId = 0
        that.receive.countyId = 0
      }
      return cityList
    },
    // 选择区县
    countyList () {
      let that = this
      let countyList = []
      that.cityList.forEach((city) => {
        if (city.area_id === that.receive.cityId) {
          that.receive.city = city.area_name
          countyList = city.county_list
          return
        }
      })
      if (that.receive.cityId === '0') {
        that.receive.countyId = 0
      }
      return countyList
    }
  ...
  methods: {
    ...
    inspectPhone () {
      ...
      if (myreg.test(this.receive.phone)) {
